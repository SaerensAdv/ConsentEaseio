...
export function useOverviewData() {
  ...
  const embedDetected = (analyticsQuery.data?.totalViews ?? 0) > 0 || diagnosticQuery.data?.bannerScriptDetected === true;
  const embedVerified = embedDetected || (analyticsQuery.isSuccess && diagnosticQuery.isSuccess);
  const consentModeReady = diagnosticQuery.data?.consentModeDetected === true && diagnosticQuery.data.defaultConsentSet === true;
  const isDefaultBanner = bannerQuery.data?.primaryColor === "#726CEA" && bannerQuery.data.heading === "We value your privacy";
  ...
  const actions: OverviewAction[] = [
    ...
    bannerQuery.isSuccess && isDefaultBanner ? { priority: 3, title: "Make the banner yours", description: "The active banner still uses the default design.", label: "Customize banner", href: "/dashboard/banner", icon: Cookie } : null,
